...
def label(df):
    ...
    n_stk_select = np.multiply(paramt.select_percent, len(df))    # 设置切片数
...
for section_num in paramt.section_list_sample:
    # 读取文件
    file_name = paramt.data_path + '/data_4/' + str(section_num) + '_4.csv'
    ...
    # 合并样本集
    if section_num == paramt.section_list_sample[0]:    # 第一个截面期
        data_sample = data_i
    else:    # 除第一个截面期外的
